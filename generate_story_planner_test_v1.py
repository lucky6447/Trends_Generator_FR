print("[TrendCurrent TEST] Story Planner — architecture test")
import re
import os
import subprocess
...
from prompt_story_planner_test import build_prompt
...
from story_planner import plan_story, attach_plan, StoryPlannerError



def _validate_writer_fact_units(article, planner, evidence=None):
    """Validate fact_ids as provenance annotations, not literal text matches.

    Semantic support is checked by Fact Guard; this function only verifies that
    each paragraph is annotated with real facts from the planner's writer set.
    """
    if not isinstance(article, dict):
        return False, ["article is not an object"]
    paragraphs = article.get("paragraphs")
    if not isinstance(paragraphs, list) or not paragraphs:
        return False, ["missing paragraphs"]

    locked_ids = {
        str(f.get("id"))
        for f in (evidence or {}).get("facts", [])
        if isinstance(f, dict) and f.get("id")
    }
    planner_ids = set(str(x) for x in planner.get("writer_fact_order", []))
    allowed = planner_ids & locked_ids if locked_ids else planner_ids

    errors = []
    for i, p in enumerate(paragraphs, 1):
        if not isinstance(p, dict):
            errors.append(f"paragraph {i} is not an object")
            continue
        if not str(p.get("text", "")).strip():
            errors.append(f"paragraph {i} is empty")
        ids = p.get("fact_ids")
        if not isinstance(ids, list) or not ids:
            errors.append(f"paragraph {i} has no fact_ids")
            continue
        bad = [str(x) for x in ids if str(x) not in allowed]
        if bad:
            errors.append(f"paragraph {i} invalid fact_ids={bad}")
    return (not errors), errors


def _reconcile_writer_fact_units(article, evidence, planner):
    """Semantically map each written paragraph to the selected locked facts.

    fact_ids are provenance metadata. They must identify facts that collectively
    support the paragraph; exact wording overlap is neither required nor used.
    Unsupported paragraphs are removed rather than given a misleading ID.
    """
    if not isinstance(article, dict):
        raise Exception("Article is not an object")

    facts = evidence.get("facts", []) if isinstance(evidence, dict) else []
    allowed_ids = [str(x) for x in planner.get("writer_fact_order", [])]
    selected = [
        f for f in facts
        if isinstance(f, dict) and str(f.get("id")) in set(allowed_ids)
    ]
    paragraphs = article.get("paragraphs", [])
    if not selected or not paragraphs:
        return article

    fact_rows = "\n".join(
        f"FACT {f.get('id')}: {json.dumps(f, ensure_ascii=False, separators=(',', ':'))}"
        for f in selected
    )
    paragraph_rows = "\n".join(
        f"PARAGRAPH {i}: {p.get('text','') if isinstance(p,dict) else p}"
        for i, p in enumerate(paragraphs, 1)
    )

    reconcile_prompt = f"""
You are a strict factual provenance editor.

The article text has already passed the main Fact Guard. Your task is ONLY to
assign provenance IDs to each paragraph from the SELECTED WRITER FACTS below.

SELECTED WRITER FACTS:
{fact_rows}
...
RULES:
- For each paragraph, return the IDs of ALL selected facts that collectively
  support every material factual claim in that paragraph.
- Support is SEMANTIC, not literal. Different wording, paraphrase, grammar,
  or sentence structure is allowed.
- Do NOT require the paragraph to repeat fact wording.
- Do NOT assign an ID merely because it concerns the same person, company,
  place, topic, or event.
- Do NOT use facts outside the selected writer facts.
- If a paragraph contains a material claim that cannot be supported by the
  selected facts, mark that paragraph for deletion.
...
"""
    ...
    response = chat(
        ...
        options={
            ...
            "top_k": 30,
            "num_ctx": 8192,
            "num_predict": max(180, min(500, 80 + len(paragraphs) * 45)),
            "num_batch": 256,
            "num_thread": 16,
        },
        format={
    ...
    mapping = result.get("paragraphs", [])
    if not isinstance(mapping, list) or len(mapping) != len(paragraphs):
        raise Exception("Writer fact reconciliation returned invalid paragraph mapping")

    selected_id_set = set(allowed_ids)
    reconciled = []
    for p, m in zip(paragraphs, mapping):
        if not isinstance(p, dict) or not isinstance(m, dict):
            continue
        ids = [str(x) for x in m.get("fact_ids", []) if str(x) in selected_id_set]
        if m.get("keep") is True and ids:
            q = dict(p)
            q["fact_ids"] = list(dict.fromkeys(ids))
            reconciled.append(q)

    article["paragraphs"] = reconciled
    if not reconciled:
        raise Exception("Writer fact reconciliation removed all unsupported paragraphs")

    print(
        f"[WRITER FACT RECONCILE] PASS | "
        f"paragraphs={len(reconciled)}/{len(paragraphs)}"
    )
    return article


def _writer_quality_check(article):
    """Conservative deterministic check for obvious filler/inference phrases."""
    paragraphs = article.get("paragraphs", []) or []
    paragraph_text = " ".join(
        str(x.get("text", "")) if isinstance(x, dict) else str(x)
        for x in paragraphs
    )
    text = (str(article.get("description", "")) + " " + paragraph_text).lower()
    bad = [
        "this highlights", "this underscores", "highlighting the significance",
        "marking an emotional moment", "aimed at helping", "significant milestone",
        "important milestone", "major implications", "could improve", "may help",
        "is expected to", "is set to", "underscores the importance",
        "reflects the significance", "demonstrates the importance",
    ]
    return [p for p in bad if p in text]
    ...
    for paragraph in article["paragraphs"]:
        if isinstance(paragraph, dict):
            if not str(paragraph.get("text", "")).strip():
                raise Exception("Invalid paragraph text")
            if not isinstance(paragraph.get("fact_ids"), list) or not paragraph.get("fact_ids"):
                raise Exception("Invalid paragraph fact_ids")
        elif not isinstance(paragraph, str) or not paragraph.strip():
...
def _generate_structured_writer(prompt, evidence):
    """Strict writer: Ollama must return paragraph objects with fact_ids."""
    schema = {
        "type": "object",
        "properties": {
            "title": {"type": "string"},
...
For every paragraph:
- "text" is the actual news paragraph.
- "fact_ids" contains ONLY the locked fact IDs that directly support every material claim in that paragraph.
- Every factual sentence must be supportable by its listed fact_ids.
- Never use a fact ID merely because it is related to the topic.
- Never combine facts into a new inference, causal claim, prediction, interpretation or significance claim.
- If a fact cannot be written without inference, omit it.
- Do not write filler, generic context, source-digest prose, or conclusions.
- Do not write source names/headlines unless attribution is itself a verified part of the fact.
- Keep the article concise and concrete.
"""
    ...
    if not isinstance(article, dict):
        raise Exception("Writer returned non-object JSON")
    return article
...
def generate_valid_article(prompt, fact_guard_source, reference_date, trend, max_attempts=2, prelocked_evidence=None):
    ...
    for i in range(max_attempts):
        try:
            ...
            article = _generate_structured_writer(prompt, generation_evidence)
    ...
    for trend in evidence_candidate_pool:
        ...
        try:
            ...
            try:
                ...
                story_plan = plan_story(keyword, evidence_lock)
                trend["_story_planner"] = story_plan
                evidence_lock = attach_plan(evidence_lock, story_plan)
            ...
            # fact_ids are provenance metadata. Reconcile them semantically
            # against the planner-selected writer facts before validating them.
            article = _reconcile_writer_fact_units(
                article,
                evidence_lock,
                evidence_lock.get("story_planner", {}),
            )

            writer_ok, writer_errors = _validate_writer_fact_units(
                article,
                evidence_lock.get("story_planner", {}),
                evidence_lock,
            )
            if not writer_ok:
                print(f"[WRITER FACT UNITS] DROP | {keyword} | errors={writer_errors}")
                continue

            writer_flags = _writer_quality_check(article)
            if writer_flags:
                print(
                    f"[WRITER QUALITY] DROP | {keyword} | "
                    f"unsupported filler/inference phrases={writer_flags}"
                )
                continue
            article["paragraphs"] = [
                p["text"] for p in article.get("paragraphs", [])
            ]
            print(f"[WRITER FACT UNITS] PASS | {keyword}")
            print(f"[WRITER QUALITY] PASS | {keyword}")
            ...
            print(f"[STORY PLANNER] DROP | {keyword} | {e}")
        except Exception as e:
